...
public sealed class WorkflowTest : WorkflowTest<RestaurantWorkflow, DataPizza>
{
    ...
    public async Task OrderPizza()
    {
        ...
        var productState = (await _dbContext.Products.AsNoTracking().FirstOrDefaultAsync(x => x.Id == _selectedProductId))!.ProductState;

        while (productState != ProductState.Terminated || productState != ProductState.Ready)
        {
            Thread.Sleep(2000);
            productState = (await _dbContext.Products.AsNoTracking().FirstOrDefaultAsync(x => x.Id == _selectedProductId))!.ProductState;
        ...
        else
        {
            ...
            while (courier.State != CourierState.Done || courier.State != CourierState.Terminated || courier.State != CourierState.Free)
            {
                Thread.Sleep(2000);
                courier = (await _dbContext.Couriers.AsNoTracking().FirstOrDefaultAsync(x => x.Id == _selectedCourierId))!;
            
                if (courier.State != CourierState.Done || courier.State != CourierState.Terminated || courier.State != CourierState.Free)
                {
                    break;
                }
            }
            ...
            {
                _testOutputHelper.WriteLine($"Курьер: {courier.Name} не успел доставить продукт \"{product.Name}\" для клиента \"{client.Name}\".");
            }
            else
            {
                product = await _dbContext.Products.AsNoTracking().FirstOrDefaultAsync(x => x.Name == creatingProduct.Name);
                ...
                _testOutputHelper.WriteLine($"Товар: {product.Name}; Клиент: {client.Name} - id: {client.Id}; Курьер: {courier.Name} - id: {courier.Id}");
            }
        }
    }
    ...
    [Fact]
    public async Task OrderPizza3()
    {
        using var _dbContext = _provider!.GetRequiredService<AppDbContext>();
        var _dbService = _provider!.GetService<DBService>();
        
        int _selectedCourierId = 5;
        int _selectedProductId;
        int _selectedClientId = 4;
        
        var creatingProduct = new Product { Name = "Сезонная (Весна)" };
